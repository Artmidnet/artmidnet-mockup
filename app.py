...
def detect_red_dot(img: Image.Image) -> tuple:
    ...
    if len(xs) == 0:
        h, w = arr.shape[:2]
        cx, cy = w // 2, h // 2
        print(f"V17 detect_red_dot: NOT FOUND — using center ({cx},{cy})", flush=True)
        return cx, cy
    cx = int(np.mean(xs))
    cy = int(np.mean(ys))
    print(f"V17 detect_red_dot: found {len(xs)} red pixels | center=({cx},{cy})", flush=True)
    return cx, cy
...
def apply_new_mockup(
    ...
    if size_px > max_size:
        print(f"V22 size_px capped: {size_px} → {max_size} (mockup={img_w}x{img_h})", flush=True)
        size_px = max_size

    pw, ph = painting_img.size
    ar = ph / pw
    print(f"V22 painting: {pw}x{ph} AR={ar:.3f} | size_px={size_px}", flush=True)
    ...
    print(f"V22 white canvas: {wc_w}x{wc_h} | ImagePoint=({img_cx},{img_cy})", flush=True)
    ...
    print(f"V22 done: paste=({paste_x},{paste_y}) size={wc_w}x{wc_h}", flush=True)
...
# ═════════════════════════════════════════════
# RECEIPT: Gmail Sender (V59 — plain email, no PDF)
# ═════════════════════════════════════════════
...
def send_receipt_email(to_email: str, subject: str, order_number: str):
    ...
    if not gmail_user or not gmail_pass:
        print("V59 send_receipt_email: ERROR — GMAIL_USER or GMAIL_APP_PASS not set", flush=True)
        return

    try:
        print(f"V59 send_receipt_email: building message...", flush=True)
        ...
        body = f"הזמנה מספר {order_number} התקבלה בהצלחה. תודה על רכישתך!"
        msg.attach(MIMEText(body, "plain", "utf-8"))

        print(f"V59 send_receipt_email: connecting to smtp.gmail.com...", flush=True)

        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
            print(f"V59 send_receipt_email: logging in...", flush=True)
            server.login(gmail_user, gmail_pass)
            print(f"V59 send_receipt_email: sending...", flush=True)
...
# ═════════════════════════════════════════════
# ENDPOINT: RECEIPT
# V59: daemon=False + join(timeout=55) — thread must finish before server sleeps
# ═════════════════════════════════════════════
...
def receipt():
    try:
        ...
        return jsonify({
            "status": "ok",
            "message": f"Receipt processed for {to_email}",
